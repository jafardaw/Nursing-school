...
class _EngineeringDashboardState extends State<EngineeringDashboard> {
          ...
          builder: (context, complaintState) {
            return BlocBuilder<StockCubit, StockState>(
              ...
              builder: (context, stockState) {
                if (complaintState is ComplaintsLoading)
  ...
  Widget _buildDashboard(
    ComplaintsLoaded complaintState,
    ...
    // 🟢 إحصائيات المخزون
    final totalStock = stockState is StockLoaded
        ? stockState.totalIn - stockState.totalOut
        : 0;
    final lowStockCount = stockState is StockLoaded
        ? stockState.lowStockCount
        : 0;
      ...
      children: [
        const Column(
          ...
          children: [
            ...
            SizedBox(height: 4),
            Text(
              'نظرة عامة على شكاوى الصيانة والمخزون',
    ...
    return GridView.count(
      ...
      children: [
        StatsCard(
          ...
          gradientEndColor: const Color(0xFFFF8E8E),
        ),
        StatsCard(
          title: 'قيد الانتظار',
          value: '$pending',
        ...
        StatsCard(
          ...
          gradientEndColor: const Color(0xFFCE93D8),
        ),
      ],
    );
  }
    ...
    if (isDesktop) {
      return Row(
        ...
        children: [
          Expanded(
            flex: 3,
            child: SizedBox(height: 300, child: BarChartWidget(data: barData)),
          ),
          const SizedBox(width: 16),
          Expanded(
            flex: 2,
            child: SizedBox(height: 300, child: PieChartWidget(data: pieData)),
          ),
        ],
      );
    }
  ...
  List<PieChartData> _buildPieData(List<ComplaintModel> complaints) {
    ...
    final colors = {
      'Pending': const Color(0xFFFF6B6B),
      'In Progress': const Color(0xFF4ECDC4),
      'Resolved': const Color(0xFF50CD89),
    };
  ...
  List<ActivityItem> _buildActivities(List<ComplaintModel> complaints) {
    final sorted = List<ComplaintModel>.from(complaints)
