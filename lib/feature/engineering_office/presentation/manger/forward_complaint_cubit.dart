...
class ForwardComplaintCubit extends Cubit<ForwardComplaintState> {
  ...
  Future<void> forwardComplaint(int id, String approverRole) async {
    emit(ForwardComplaintLoading(complaintId: id));

    try {
      final complaint = await _repo.forwardComplaint(id, approverRole);
      ...
      final newStage = stageLabels[complaint.currentStageRole] ?? complaint.currentStageRole;

      emit(ForwardComplaintSuccess(
